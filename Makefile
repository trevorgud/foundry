...
.PHONY: up down restart logs dev-up dev-restart dev-logs dev-shell configure-world test-all test-engine test-state-schema test-state-schema-live test-foundry test-foundry-health test-foundry-rules test-foundry-local state state-debug state-fixtures screenshot
...
test-all: test-engine test-state-schema test-foundry

test-engine:
	$(COMPOSE_DEV) run --rm test sh -lc '$(TEST_DEPS) && npm run test:engine'

test-state-schema:
	$(COMPOSE_DEV) run --rm test sh -lc '$(TEST_DEPS) && npm run test:state:schema'

test-state-schema-live:
	$(COMPOSE_DEV) run --rm $(STATE_ENV) test sh -lc '$(TEST_DEPS) && npm run test:state:schema:live'
...
state-fixtures:
	$(COMPOSE_DEV) run --rm $(STATE_ENV) test sh -lc '$(TEST_DEPS) && npm run foundry:state:fixtures'
